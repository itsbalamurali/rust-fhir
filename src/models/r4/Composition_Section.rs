...
impl Composition_Section<'_> {
    ...
    /// instead represents something or someone associated with the subject such as (for
    /// a patient subject) a spouse, parent, fetus, or donor. If not focus is specified,
    /// the focus is assumed to be focus of the parent section, or, for a section in
    /// the Composition itself, the subject of the composition. Sections with a focus
    /// SHALL only include resources where the logical subject (patient, subject, focus,
    /// etc.) matches the section focus, or the resources have no logical subject (few
    /// resources).
    ...
    /// of items from another source, or whether it is a prepared list where items may be
