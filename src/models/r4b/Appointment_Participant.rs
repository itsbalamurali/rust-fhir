...
impl Appointment_Participant<'_> {
    ...
    /// a use-case where two doctors need to meet to discuss the results for a specific
    /// patient, and the patient is not required to be present.
    pub fn required(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("required") {
            return Some(string);
        }
        return None;
    }
...
impl Appointment_ParticipantBuilder {
    ...
    pub fn required<'a>(&'a mut self, val: &str) -> &'a mut Appointment_ParticipantBuilder {
        self.value["required"] = json!(val);
        return self;
    }

    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut Appointment_ParticipantBuilder {
