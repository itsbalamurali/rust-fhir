...
impl BiologicallyDerivedProduct_Storage<'_> {
    ...
    pub fn scale(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("scale") {
            return Some(string);
        }
        return None;
    }
...
impl BiologicallyDerivedProduct_StorageBuilder {
    ...
    pub fn scale<'a>(&'a mut self, val: &str) -> &'a mut BiologicallyDerivedProduct_StorageBuilder {
        self.value["scale"] = json!(val);
        return self;
    }
