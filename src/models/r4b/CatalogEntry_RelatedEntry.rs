...
impl CatalogEntry_RelatedEntry<'_> {
    ...
    pub fn relationtype(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("relationtype") {
            return Some(string);
        }
        return None;
    }
...
impl CatalogEntry_RelatedEntryBuilder {
    ...
    pub fn relationtype<'a>(&'a mut self, val: &str) -> &'a mut CatalogEntry_RelatedEntryBuilder {
        self.value["relationtype"] = json!(val);
        return self;
    }
}
