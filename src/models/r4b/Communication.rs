...
use crate::models::r4b::Communication_Payload::Communication_Payload;
...
impl Communication<'_> {
    ...
    /// device) which was the target of the communication. If receipts need to be tracked
    /// by an individual, a separate resource instance will need to be created for each
    /// recipient.  Multiple recipient communications are intended where either receipts
    /// are not tracked (e.g. a mass mail-out) or a receipt is captured in aggregate (all
