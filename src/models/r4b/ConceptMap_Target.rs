...
use crate::models::r4b::ConceptMap_DependsOn::ConceptMap_DependsOn;
...
impl ConceptMap_Target<'_> {
    ...
    pub fn equivalence(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("equivalence") {
            return Some(string);
        }
        return None;
    }
...
impl ConceptMap_TargetBuilder {
    ...
    pub fn equivalence<'a>(&'a mut self, val: &str) -> &'a mut ConceptMap_TargetBuilder {
        self.value["equivalence"] = json!(val);
        return self;
    }
