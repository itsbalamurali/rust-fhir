...
use crate::models::r4b::DeviceMetric_Calibration::DeviceMetric_Calibration;
...
impl DeviceMetric<'_> {
    ...
    pub fn category(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("category") {
            return Some(string);
        }
        return None;
    }
    ...
    /// hours. An example for a measurement repetition time in the range of milliseconds
    /// is the sampling rate of an ECG. An example for a measurement repetition time
    /// in the range of hours is a NIBP that is triggered automatically every hour. The
    /// update period may be different than the measurement repetition time, if the device
    /// does not update the published observed value with the same frequency as it was
    /// measured.
    ...
    /// etc.
    pub fn operational_status(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("operationalStatus") {
            return Some(string);
        }
        return None;
    }
    ...
    /// structure of the parent Device. An example would be a Device that represents
    /// a Channel. This reference can be used by a client application to distinguish
...
impl DeviceMetricBuilder {
    ...
    pub fn category<'a>(&'a mut self, val: &str) -> &'a mut DeviceMetricBuilder {
        self.value["category"] = json!(val);
        return self;
    }

    pub fn color<'a>(&'a mut self, val: &str) -> &'a mut DeviceMetricBuilder {
    ...
    pub fn operational_status<'a>(&'a mut self, val: &str) -> &'a mut DeviceMetricBuilder {
        self.value["operationalStatus"] = json!(val);
        return self;
    }
