...
use crate::models::r4b::DocumentManifest_Related::DocumentManifest_Related;
...
impl DocumentManifest<'_> {
    ...
    /// necessarily the same thing as the actual resource last modified time, since it may
    /// be modified, replicated, etc.).
    ...
    /// that share a common exposure). If the documents cross more than one subject, then
...
impl DocumentManifestBuilder {
    ...
    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut DocumentManifestBuilder {
