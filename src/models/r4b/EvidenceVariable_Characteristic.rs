...
use crate::models::r4b::EvidenceVariable_TimeFromStart::EvidenceVariable_TimeFromStart;
...
impl EvidenceVariable_Characteristic<'_> {
    ...
    /// entry.
    pub fn time_from_start(&self) -> Option<EvidenceVariable_TimeFromStart> {
        if let Some(val) = self.value.get("timeFromStart") {
            return Some(EvidenceVariable_TimeFromStart {
                value: Cow::Borrowed(val),
            });
        }
        return None;
...
impl EvidenceVariable_CharacteristicBuilder {
    ...
    pub fn time_from_start<'a>(
        &'a mut self,
        val: EvidenceVariable_TimeFromStart,
    ) -> &'a mut EvidenceVariable_CharacteristicBuilder {
