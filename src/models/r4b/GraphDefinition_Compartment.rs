...
impl GraphDefinition_Compartment<'_> {
    ...
    pub fn rule(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("rule") {
            return Some(string);
        }
        return None;
    }
...
impl GraphDefinition_CompartmentBuilder {
    ...
    pub fn rule<'a>(&'a mut self, val: &str) -> &'a mut GraphDefinition_CompartmentBuilder {
        self.value["rule"] = json!(val);
        return self;
    }

    pub fn fhir_use<'a>(&'a mut self, val: &str) -> &'a mut GraphDefinition_CompartmentBuilder {
