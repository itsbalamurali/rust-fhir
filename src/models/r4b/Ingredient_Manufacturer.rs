...
impl Ingredient_Manufacturer<'_> {
    ...
    /// The way in which this manufacturer is associated with the ingredient. For example
    /// whether it is a possible one (others allowed), or an exclusive authorized one for
    /// this ingredient. Note that this is not the manufacturing process role.
...
impl Ingredient_ManufacturerBuilder {
    ...
    pub fn _role<'a>(&'a mut self, val: Element) -> &'a mut Ingredient_ManufacturerBuilder {
    ...
    pub fn role<'a>(&'a mut self, val: &str) -> &'a mut Ingredient_ManufacturerBuilder {
