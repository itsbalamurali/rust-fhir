...
impl MarketingStatus<'_> {
    ...
    /// chain.
    pub fn date_range(&self) -> Option<Period> {
        if let Some(val) = self.value.get("dateRange") {
            return Some(Period {
                value: Cow::Borrowed(val),
            });
        }
    ...
    pub fn validate(&self) -> bool {
        ...
        if let Some(_val) = self.date_range() {
            if !_val.validate() {
                return false;
            }
        }
...
impl MarketingStatusBuilder {
    ...
    pub fn new(status: CodeableConcept) -> MarketingStatusBuilder {
    ...
    pub fn country<'a>(&'a mut self, val: CodeableConcept) -> &'a mut MarketingStatusBuilder {
        self.value["country"] = json!(val.value);
        return self;
    }

    pub fn date_range<'a>(&'a mut self, val: Period) -> &'a mut MarketingStatusBuilder {
        self.value["dateRange"] = json!(val.value);
        return self;
    }
