...
use crate::models::r4b::MessageDefinition_AllowedResponse::MessageDefinition_AllowedResponse;
use crate::models::r4b::MessageDefinition_Focus::MessageDefinition_Focus;
...
impl MessageDefinition<'_> {
    ...
    /// the message definition changes.
    ...
    /// canonical reference to a [GraphDefinition](graphdefinition.html) that it controls
    /// what resources are to be added to the bundle when building the document. The
    ...
    /// Explanation of why this message definition is needed and why it has been designed
    /// as it has.
    ...
    pub fn response_required(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("responseRequired") {
            return Some(string);
        }
        return None;
    }
    ...
    /// arbitrary value managed by the message definition author and is not expected to be
...
impl MessageDefinitionBuilder {
    ...
    pub fn category<'a>(&'a mut self, val: &str) -> &'a mut MessageDefinitionBuilder {
    ...
    pub fn response_required<'a>(&'a mut self, val: &str) -> &'a mut MessageDefinitionBuilder {
        self.value["responseRequired"] = json!(val);
        return self;
    }

    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut MessageDefinitionBuilder {
