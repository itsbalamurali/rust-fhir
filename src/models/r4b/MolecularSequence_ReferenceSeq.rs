...
impl MolecularSequence_ReferenceSeq<'_> {
    ...
    /// of replication ([SO:0000340](http://www.sequenceontology.org/browser/current_svn/
    /// term/SO:0000340)).
    ...
    pub fn orientation(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("orientation") {
            return Some(string);
        }
        return None;
    }
    ...
    /// identifies reference sequence for genes, “NM_” for messenger RNA transcripts, and
    ...
    /// on the short arm of the chromosome, and the Crick strand as the one whose 5'-end
    /// is on the long arm.
    pub fn strand(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("strand") {
            return Some(string);
        }
        return None;
    }
...
impl MolecularSequence_ReferenceSeqBuilder {
    ...
    pub fn orientation<'a>(
        ...
        self.value["orientation"] = json!(val);
        return self;
    }
    ...
    pub fn strand<'a>(&'a mut self, val: &str) -> &'a mut MolecularSequence_ReferenceSeqBuilder {
        self.value["strand"] = json!(val);
        return self;
    }
