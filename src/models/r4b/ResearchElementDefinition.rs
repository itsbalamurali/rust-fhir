...
use crate::models::r4b::ResearchElementDefinition_Characteristic::ResearchElementDefinition_Characteristic;
...
impl ResearchElementDefinition<'_> {
    ...
    /// The period during which the research element definition content was or is planned
    /// to be in active use.
    ...
    /// A legal or geographic region in which the research element definition is intended
    /// to be used.
    ...
    /// reference. It SHALL remain the same when the research element definition is stored
    ...
    /// to assist with indexing and searching for appropriate research element definition
    /// instances.
    ...
    pub fn variable_type(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("variableType") {
            return Some(string);
        }
        return None;
    }
    ...
    /// is not expected to be globally unique. For example, it might be a timestamp (e.g.
...
impl ResearchElementDefinitionBuilder {
    ...
    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut ResearchElementDefinitionBuilder {
    ...
    pub fn fhir_type<'a>(&'a mut self, val: &str) -> &'a mut ResearchElementDefinitionBuilder {
    ...
    pub fn variable_type<'a>(&'a mut self, val: &str) -> &'a mut ResearchElementDefinitionBuilder {
        self.value["variableType"] = json!(val);
        return self;
    }
