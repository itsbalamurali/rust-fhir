...
use crate::models::r4b::StructureMap_Parameter::StructureMap_Parameter;
use serde_json::json;
...
impl StructureMap_Target<'_> {
    ...
    pub fn context_type(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("contextType") {
            return Some(string);
        }
        return None;
    }
...
impl StructureMap_TargetBuilder {
    ...
    pub fn context_type<'a>(&'a mut self, val: &str) -> &'a mut StructureMap_TargetBuilder {
        self.value["contextType"] = json!(val);
        return self;
    }
