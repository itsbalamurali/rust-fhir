...
use crate::models::r4b::SubscriptionStatus_NotificationEvent::SubscriptionStatus_NotificationEvent;
use serde_json::json;
...
impl SubscriptionStatus<'_> {
    ...
    /// The total number of actual events which have been generated since the Subscription
    /// was created (inclusive of this notification) - regardless of how many have been
    /// successfully communicated.  This number is NOT incremented for handshake and
    /// heartbeat notifications.
    pub fn events_since_subscription_start(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("eventsSinceSubscriptionStart") {
            return Some(string);
        }
        return None;
    }
