...
use crate::models::r4b::TestScript_RequestHeader::TestScript_RequestHeader;
use serde_json::json;
...
impl TestScript_Operation<'_> {
    ...
    pub fn method(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("method") {
            return Some(string);
        }
        return None;
    }
...
impl TestScript_OperationBuilder {
    ...
    pub fn method<'a>(&'a mut self, val: &str) -> &'a mut TestScript_OperationBuilder {
        self.value["method"] = json!(val);
        return self;
    }
