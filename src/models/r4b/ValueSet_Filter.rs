...
impl ValueSet_Filter<'_> {
    ...
    pub fn op(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("op") {
            return Some(string);
        }
        return None;
    }
    ...
    /// filter represents a filter defined in CodeSystem) when the operation is 'regex',
...
impl ValueSet_FilterBuilder {
    ...
    pub fn op<'a>(&'a mut self, val: &str) -> &'a mut ValueSet_FilterBuilder {
