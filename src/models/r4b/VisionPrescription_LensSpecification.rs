...
use crate::models::r4b::VisionPrescription_Prism::VisionPrescription_Prism;
use serde_json::json;
...
impl VisionPrescription_LensSpecification<'_> {
    ...
    pub fn eye(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("eye") {
            return Some(string);
        }
        return None;
    }
...
impl VisionPrescription_LensSpecificationBuilder {
    ...
    pub fn eye<'a>(&'a mut self, val: &str) -> &'a mut VisionPrescription_LensSpecificationBuilder {
        self.value["eye"] = json!(val);
        return self;
    }
