...
use crate::models::r5::Account_Coverage::Account_Coverage;
use crate::models::r5::Account_Guarantor::Account_Guarantor;
...
impl Account<'_> {
    ...
    /// The BillingStatus tracks the lifecycle of the account through the billing process.
    /// It indicates how transactions are treated when they are allocated to the account.
    pub fn billing_status(&self) -> Option<CodeableConcept> {
        if let Some(val) = self.value.get("billingStatus") {
    ...
    pub fn validate(&self) -> bool {
        ...
        if let Some(_val) = self.billing_status() {
            if !_val.validate() {
                return false;
            }
        }
...
impl AccountBuilder {
    ...
    pub fn billing_status<'a>(&'a mut self, val: CodeableConcept) -> &'a mut AccountBuilder {
        self.value["billingStatus"] = json!(val.value);
        return self;
    }
    ...
    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut AccountBuilder {
