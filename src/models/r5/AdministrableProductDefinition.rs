...
use crate::models::r5::AdministrableProductDefinition_Property::AdministrableProductDefinition_Property;
use crate::models::r5::AdministrableProductDefinition_RouteOfAdministration::AdministrableProductDefinition_RouteOfAdministration;
...
impl AdministrableProductDefinition<'_> {
    ...
    /// element is not a reference to the item(s) that make up this administrable form
    /// (for which see AdministrableProductDefinition.producedFrom). It is medicinal
    /// product as a whole, which may have several components (as well as packaging,
    /// devices etc.), that are given to the patient in this final administrable form. A
    /// single medicinal product may have several different administrable products (e.g.
    /// a tablet and a cream), and these could have different administrable forms (e.g.
    /// tablet as oral solid, or tablet crushed).
    ...
    /// The constituent manufactured item(s) that this administrable product is produced
    /// from. Either a single item, or several that are mixed before administration
    /// (e.g. a power item and a solvent item, to make a consumable solution). Note the
    /// items this is produced from are not raw ingredients (see
    /// AdministrableProductDefinition.ingredient), but manufactured medication items
    /// (ManufacturedItemDefinitions), which may be combined or prepared and transformed
    /// for patient use. The constituent items that this administrable form is produced
    /// from are all part of the product (for which see
