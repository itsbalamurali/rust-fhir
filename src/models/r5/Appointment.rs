...
use crate::models::r5::Appointment_Participant::Appointment_Participant;
...
impl Appointment<'_> {
    ...
    /// reporting/billing/futher processing to determine if further actions are required,
    /// or specific fees apply.
    pub fn cancellation_reason(&self) -> Option<CodeableConcept> {
        if let Some(val) = self.value.get("cancellationReason") {
            return Some(CodeableConcept {
                value: Cow::Borrowed(val),
            });
        }
    ...
    /// the resource was created on the FHIR server, and should remain unchanged over the
    ...
    /// meeting request, or appointment list. Detailed or expanded information should be
    ...
    /// but any time would work.  Also, if there is, for example, a planned 15 minute
    /// break in the middle of a long appointment, the duration may be 15 minutes less
    ...
    /// before).
    pub fn patient_instruction(&self) -> Option<Vec<CodeableReference>> {
        if let Some(Value::Array(val)) = self.value.get("patientInstruction") {
            return Some(
                val.into_iter()
    ...
    /// administrative. This can be coded, or as specified using information from another
    /// resource. When the patient arrives and the encounter begins it may be used as
    /// the admission diagnosis. The indication will typically be a Condition (with other
    ...
    pub fn validate(&self) -> bool {
        ...
        if let Some(_val) = self.cancellation_reason() {
            if !_val.validate() {
                return false;
            }
        }
...
impl AppointmentBuilder {
    ...
    pub fn account<'a>(&'a mut self, val: Vec<Reference>) -> &'a mut AppointmentBuilder {
    ...
    pub fn cancellation_reason<'a>(
        &'a mut self,
        val: CodeableConcept,
    ) -> &'a mut AppointmentBuilder {
        self.value["cancellationReason"] = json!(val.value);
        return self;
    }
    ...
    pub fn note<'a>(&'a mut self, val: Vec<Annotation>) -> &'a mut AppointmentBuilder {
        self.value["note"] = json!(val.into_iter().map(|e| e.value).collect::<Vec<_>>());
        return self;
    }

    pub fn patient_instruction<'a>(
        &'a mut self,
        val: Vec<CodeableReference>,
    ) -> &'a mut AppointmentBuilder {
        self.value["patientInstruction"] =
            json!(val.into_iter().map(|e| e.value).collect::<Vec<_>>());
        return self;
    }

    pub fn priority<'a>(&'a mut self, val: CodeableConcept) -> &'a mut AppointmentBuilder {
        self.value["priority"] = json!(val.value);
        return self;
    }

    pub fn reason<'a>(&'a mut self, val: Vec<CodeableReference>) -> &'a mut AppointmentBuilder {
        self.value["reason"] = json!(val.into_iter().map(|e| e.value).collect::<Vec<_>>());
        return self;
    }

    pub fn replaces<'a>(&'a mut self, val: Vec<Reference>) -> &'a mut AppointmentBuilder {
    ...
    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut AppointmentBuilder {
        self.value["status"] = json!(val);
        return self;
    }

    pub fn subject<'a>(&'a mut self, val: Reference) -> &'a mut AppointmentBuilder {
