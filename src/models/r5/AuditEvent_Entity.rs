...
use crate::models::r5::AuditEvent_Detail::AuditEvent_Detail;
...
impl AuditEvent_Entity<'_> {
    ...
    /// that entity in the activity. This is most used to indicate when persistence media
    /// (the entity) are used by an agent. For example when importing data from a device,
    /// the device would be described in an entity, and the user importing data from that
    /// media would be indicated as the entity.agent.
    pub fn agent(&self) -> Option<Vec<AuditEvent_Agent>> {
...
impl AuditEvent_EntityBuilder {
    ...
    pub fn agent<'a>(&'a mut self, val: Vec<AuditEvent_Agent>) -> &'a mut AuditEvent_EntityBuilder {
    ...
    pub fn role<'a>(&'a mut self, val: CodeableConcept) -> &'a mut AuditEvent_EntityBuilder {
