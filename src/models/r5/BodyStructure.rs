...
use crate::models::r5::BodyStructure_ExcludedStructure::BodyStructure_ExcludedStructure;
use crate::models::r5::BodyStructure_IncludedStructure::BodyStructure_IncludedStructure;
...
impl BodyStructure<'_> {
    ...
    pub fn excluded_structure(&self) -> Option<Vec<BodyStructure_ExcludedStructure>> {
        if let Some(Value::Array(val)) = self.value.get("excludedStructure") {
            return Some(
                val.into_iter()
                    .map(|e| BodyStructure_ExcludedStructure {
                        value: Cow::Borrowed(e),
                    })
                    .collect::<Vec<_>>(),
            );
    ...
    /// structure.
    pub fn included_structure(&self) -> Vec<BodyStructure_IncludedStructure> {
        self.value
            .get("includedStructure")
            .unwrap()
            .as_array()
            .unwrap()
            .into_iter()
            .map(|e| BodyStructure_IncludedStructure {
                value: Cow::Borrowed(e),
            })
            .collect::<Vec<_>>()
    }
    ...
    pub fn validate(&self) -> bool {
        ...
        if let Some(_val) = self.excluded_structure() {
            if !_val.into_iter().map(|e| e.validate()).all(|x| x == true) {
                return false;
            }
        }
...
impl BodyStructureBuilder {
    ...
    pub fn new(
        included_structure: Vec<BodyStructure_IncludedStructure>,
        patient: Reference,
    ) -> BodyStructureBuilder {
        let mut __value: Value = json!({});
        __value["includedStructure"] = json!(included_structure
            .into_iter()
            .map(|e| e.value)
            .collect::<Vec<_>>());
    ...
    pub fn excluded_structure<'a>(
        &'a mut self,
        val: Vec<BodyStructure_ExcludedStructure>,
    ) -> &'a mut BodyStructureBuilder {
        self.value["excludedStructure"] =
