...
impl CarePlan_Activity<'_> {
    ...
    /// be patient education, exercise, or a medication administration. The reference
    /// to an "event" resource, such as Procedure or Encounter or Observation,
    /// represents the activity that was performed. The requested activity can
    /// be conveyed using CarePlan.activity.plannedActivityDetail OR using the
    /// CarePlan.activity.plannedActivityReference (a reference to a “request” resource).
