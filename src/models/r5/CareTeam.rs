...
use crate::models::r5::CareTeam_Participant::CareTeam_Participant;
...
/// The Care Team includes all the people and organizations who plan to participate in
...
impl CareTeam<'_> {
    ...
    /// co-existing teams, such as care plan team, episode of care team, longitudinal
    /// care team.
...
impl CareTeamBuilder {
    ...
    pub fn reason<'a>(&'a mut self, val: Vec<CodeableReference>) -> &'a mut CareTeamBuilder {
        self.value["reason"] = json!(val.into_iter().map(|e| e.value).collect::<Vec<_>>());
        return self;
    }

    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut CareTeamBuilder {
