...
#[derive(Debug)]
pub struct Citation_StatusDate<'a> {
    pub(crate) value: Cow<'a, Value>,
}

impl Citation_StatusDate<'_> {
    pub fn new(value: &Value) -> Citation_StatusDate {
        Citation_StatusDate {
...
#[derive(Debug)]
pub struct Citation_StatusDateBuilder {
    pub(crate) value: Value,
}

impl Citation_StatusDateBuilder {
    pub fn build(&self) -> Citation_StatusDate {
        Citation_StatusDate {
            value: Cow::Owned(self.value.clone()),
        }
    }

    pub fn with(existing: Citation_StatusDate) -> Citation_StatusDateBuilder {
        Citation_StatusDateBuilder {
            value: (*existing.value).clone(),
        }
    }

    pub fn new(activity: CodeableConcept, period: Period) -> Citation_StatusDateBuilder {
        let mut __value: Value = json!({});
        __value["activity"] = json!(activity.value);
        __value["period"] = json!(period.value);
        return Citation_StatusDateBuilder { value: __value };
    }

    pub fn _actual<'a>(&'a mut self, val: Element) -> &'a mut Citation_StatusDateBuilder {
        self.value["_actual"] = json!(val.value);
        return self;
    }

    pub fn actual<'a>(&'a mut self, val: bool) -> &'a mut Citation_StatusDateBuilder {
        self.value["actual"] = json!(val);
        return self;
    }

    pub fn extension<'a>(&'a mut self, val: Vec<Extension>) -> &'a mut Citation_StatusDateBuilder {
        self.value["extension"] = json!(val.into_iter().map(|e| e.value).collect::<Vec<_>>());
        return self;
    }

    pub fn id<'a>(&'a mut self, val: &str) -> &'a mut Citation_StatusDateBuilder {
    ...
    pub fn modifier_extension<'a>(
        &'a mut self,
        val: Vec<Extension>,
    ) -> &'a mut Citation_StatusDateBuilder {
