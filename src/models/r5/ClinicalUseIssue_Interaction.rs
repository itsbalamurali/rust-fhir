...
use crate::models::r5::ClinicalUseIssue_Interactant::ClinicalUseIssue_Interactant;
...
#[derive(Debug)]
pub struct ClinicalUseIssue_Interaction<'a> {
    pub(crate) value: Cow<'a, Value>,
}

impl ClinicalUseIssue_Interaction<'_> {
    pub fn new(value: &Value) -> ClinicalUseIssue_Interaction {
    ...
    pub fn interactant(&self) -> Option<Vec<ClinicalUseIssue_Interactant>> {
        if let Some(Value::Array(val)) = self.value.get("interactant") {
            return Some(
                val.into_iter()
                    .map(|e| ClinicalUseIssue_Interactant {
                        value: Cow::Borrowed(e),
                    })
                    .collect::<Vec<_>>(),
            );
...
#[derive(Debug)]
pub struct ClinicalUseIssue_InteractionBuilder {
    pub(crate) value: Value,
}

impl ClinicalUseIssue_InteractionBuilder {
    pub fn build(&self) -> ClinicalUseIssue_Interaction {
        ClinicalUseIssue_Interaction {
            value: Cow::Owned(self.value.clone()),
        }
    }

    pub fn with(existing: ClinicalUseIssue_Interaction) -> ClinicalUseIssue_InteractionBuilder {
        ClinicalUseIssue_InteractionBuilder {
            value: (*existing.value).clone(),
        }
    }

    pub fn new() -> ClinicalUseIssue_InteractionBuilder {
        let mut __value: Value = json!({});
        return ClinicalUseIssue_InteractionBuilder { value: __value };
    }

    pub fn effect<'a>(
        &'a mut self,
        val: CodeableReference,
    ) -> &'a mut ClinicalUseIssue_InteractionBuilder {
    ...
    pub fn id<'a>(&'a mut self, val: &str) -> &'a mut ClinicalUseIssue_InteractionBuilder {
    ...
    pub fn interactant<'a>(
        &'a mut self,
        val: Vec<ClinicalUseIssue_Interactant>,
    ) -> &'a mut ClinicalUseIssue_InteractionBuilder {
