...
use crate::models::r5::CommunicationRequest_Payload::CommunicationRequest_Payload;
...
impl CommunicationRequest<'_> {
    ...
    pub fn information_provider(&self) -> Option<Vec<Reference>> {
        if let Some(Value::Array(val)) = self.value.get("informationProvider") {
            return Some(
                val.into_iter()
                    .map(|e| Reference {
                        value: Cow::Borrowed(e),
    ...
    pub fn validate(&self) -> bool {
        ...
        if let Some(_val) = self.information_provider() {
            if !_val.into_iter().map(|e| e.validate()).all(|x| x == true) {
                return false;
            }
        }
...
impl CommunicationRequestBuilder {
    ...
    pub fn _intent<'a>(&'a mut self, val: Element) -> &'a mut CommunicationRequestBuilder {
    ...
    pub fn information_provider<'a>(
        &'a mut self,
        val: Vec<Reference>,
    ) -> &'a mut CommunicationRequestBuilder {
        self.value["informationProvider"] =
            json!(val.into_iter().map(|e| e.value).collect::<Vec<_>>());
        return self;
    }

    pub fn intent<'a>(&'a mut self, val: &str) -> &'a mut CommunicationRequestBuilder {
