...
impl CommunicationRequest_Payload<'_> {
    ...
    /// The communicated content (or for multi-part communications, one portion of the
    /// communication).
    pub fn content_codeable_concept(&self) -> Option<CodeableConcept> {
        if let Some(val) = self.value.get("contentCodeableConcept") {
    ...
    pub fn validate(&self) -> bool {
        ...
        if let Some(_val) = self.content_codeable_concept() {
            if !_val.validate() {
                return false;
            }
        }
...
impl CommunicationRequest_PayloadBuilder {
    ...
    pub fn content_codeable_concept<'a>(
        &'a mut self,
        val: CodeableConcept,
    ) -> &'a mut CommunicationRequest_PayloadBuilder {
        self.value["contentCodeableConcept"] = json!(val.value);
        return self;
    }
