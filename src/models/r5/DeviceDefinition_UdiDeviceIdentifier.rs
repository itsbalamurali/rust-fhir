...
use crate::models::r5::DeviceDefinition_MarketDistribution::DeviceDefinition_MarketDistribution;
...
impl DeviceDefinition_UdiDeviceIdentifier<'_> {
    ...
    /// The organization that assigns the identifier algorithm.
    pub fn market_distribution(&self) -> Option<Vec<DeviceDefinition_MarketDistribution>> {
        if let Some(Value::Array(val)) = self.value.get("marketDistribution") {
            return Some(
                val.into_iter()
                    .map(|e| DeviceDefinition_MarketDistribution {
                        value: Cow::Borrowed(e),
                    })
                    .collect::<Vec<_>>(),
            );
...
impl DeviceDefinition_UdiDeviceIdentifierBuilder {
    ...
    pub fn market_distribution<'a>(
        &'a mut self,
        val: Vec<DeviceDefinition_MarketDistribution>,
    ) -> &'a mut DeviceDefinition_UdiDeviceIdentifierBuilder {
        self.value["marketDistribution"] =
