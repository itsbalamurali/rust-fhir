...
impl Device_UdiCarrier<'_> {
    ...
    pub fn entry_type(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("entryType") {
            return Some(string);
        }
        return None;
    }
    ...
    /// HIBCC: http://hl7.org/fhir/NamingSystem/hibcc-diI,   3) ICCBBA for blood
    /// containers: http://hl7.org/fhir/NamingSystem/iccbba-blood-di,   4) ICCBA for other
    /// devices: http://hl7.org/fhir/NamingSystem/iccbba-other-di # Informationsstelle für
    /// Arzneispezialitäten (IFA GmbH) (EU only): http://hl7.org/fhir/NamingSystem/ifa-
    /// gmbh-di.
    ...
    /// repository uri as the system. For example, UDIs of devices managed in the U.S.
    /// by the FDA, the value is http://hl7.org/fhir/NamingSystem/us-fda-udi or in the
    /// European Union by the European Commission http://hl7.org/fhir/NamingSystem/eu-
    /// ec-udi.
...
impl Device_UdiCarrierBuilder {
    ...
    pub fn entry_type<'a>(&'a mut self, val: &str) -> &'a mut Device_UdiCarrierBuilder {
        self.value["entryType"] = json!(val);
        return self;
    }
