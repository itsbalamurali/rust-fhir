...
use crate::models::r5::DiagnosticReport_Media::DiagnosticReport_Media;
...
/// of patients, products, substances, devices, and locations, and/or specimens
/// derived from these. The report includes clinical context such as requesting
/// provider information, and some mix of atomic results, images, textual and coded
/// interpretations, and formatted representation of diagnostic reports. The report
/// also includes non-clinical context such as batch analysis and stability reporting
...
impl DiagnosticReport<'_> {
    ...
    pub fn composition(&self) -> Option<Reference> {
        if let Some(val) = self.value.get("composition") {
    ...
    /// but this is not required. A fully enabled PACS viewer can use this information to
    ...
    pub fn validate(&self) -> bool {
        ...
        if let Some(_val) = self.composition() {
            if !_val.validate() {
                return false;
            }
        }
...
impl DiagnosticReportBuilder {
    ...
    pub fn composition<'a>(&'a mut self, val: Reference) -> &'a mut DiagnosticReportBuilder {
        self.value["composition"] = json!(val.value);
        return self;
    }
    ...
    pub fn note<'a>(&'a mut self, val: Vec<Annotation>) -> &'a mut DiagnosticReportBuilder {
    ...
    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut DiagnosticReportBuilder {
