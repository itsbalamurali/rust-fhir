...
use crate::models::r5::DocumentReference_Attester::DocumentReference_Attester;
use crate::models::r5::DocumentReference_Content::DocumentReference_Content;
use crate::models::r5::DocumentReference_RelatesTo::DocumentReference_RelatesTo;
...
impl DocumentReference<'_> {
    ...
    pub fn attester(&self) -> Option<Vec<DocumentReference_Attester>> {
        if let Some(Value::Array(val)) = self.value.get("attester") {
            return Some(
                val.into_iter()
                    .map(|e| DocumentReference_Attester {
    ...
    /// of the "reference" to the document, while DocumentReference.securityLabel contains
    /// a snapshot of the security labels on the document the reference refers to.
    ...
    /// (such as a document about a herd of farm animals, or a set of patients that share
    /// a common exposure).
...
impl DocumentReferenceBuilder {
    ...
    pub fn attester<'a>(
        &'a mut self,
        val: Vec<DocumentReference_Attester>,
    ...
    pub fn based_on<'a>(&'a mut self, val: Vec<Reference>) -> &'a mut DocumentReferenceBuilder {
    ...
    pub fn encounter<'a>(&'a mut self, val: Vec<Reference>) -> &'a mut DocumentReferenceBuilder {
        self.value["encounter"] = json!(val.into_iter().map(|e| e.value).collect::<Vec<_>>());
        return self;
    }

    pub fn event<'a>(&'a mut self, val: Vec<CodeableConcept>) -> &'a mut DocumentReferenceBuilder {
    ...
    pub fn period<'a>(&'a mut self, val: Period) -> &'a mut DocumentReferenceBuilder {
    ...
    pub fn related<'a>(&'a mut self, val: Vec<Reference>) -> &'a mut DocumentReferenceBuilder {
    ...
    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut DocumentReferenceBuilder {
