...
use crate::models::r5::FamilyMemberHistory_Condition::FamilyMemberHistory_Condition;
use crate::models::r5::FamilyMemberHistory_Procedure::FamilyMemberHistory_Procedure;
...
impl FamilyMemberHistory<'_> {
    ...
    /// a repeating section to allow a system to represent more than one procedure per
    /// resource, though there is nothing stopping multiple resources - one per procedure.
    pub fn procedure(&self) -> Option<Vec<FamilyMemberHistory_Procedure>> {
        if let Some(Value::Array(val)) = self.value.get("procedure") {
            return Some(
                val.into_iter()
                    .map(|e| FamilyMemberHistory_Procedure {
                        value: Cow::Borrowed(e),
                    })
                    .collect::<Vec<_>>(),
            );
...
impl FamilyMemberHistoryBuilder {
    ...
    pub fn procedure<'a>(
        &'a mut self,
        val: Vec<FamilyMemberHistory_Procedure>,
    ) -> &'a mut FamilyMemberHistoryBuilder {
    ...
    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut FamilyMemberHistoryBuilder {
