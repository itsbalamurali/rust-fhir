...
impl ImplementationGuide_Page<'_> {
    ...
    pub fn generation(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("generation") {
            return Some(string);
        }
        return None;
    }
...
impl ImplementationGuide_PageBuilder {
    ...
    pub fn generation<'a>(&'a mut self, val: &str) -> &'a mut ImplementationGuide_PageBuilder {
        self.value["generation"] = json!(val);
        return self;
    }
