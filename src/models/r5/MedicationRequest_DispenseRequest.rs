...
use crate::models::r5::MedicationRequest_InitialFill::MedicationRequest_InitialFill;
...
impl MedicationRequest_DispenseRequest<'_> {
    ...
    pub fn dispenser(&self) -> Option<Reference> {
        if let Some(val) = self.value.get("dispenser") {
    ...
    pub fn dispenser_instruction(&self) -> Option<Vec<Annotation>> {
        if let Some(Value::Array(val)) = self.value.get("dispenserInstruction") {
            return Some(
                val.into_iter()
                    .map(|e| Annotation {
    ...
    /// medication dispense.
    pub fn dose_administration_aid(&self) -> Option<CodeableConcept> {
        if let Some(val) = self.value.get("doseAdministrationAid") {
    ...
    pub fn validate(&self) -> bool {
        ...
        if let Some(_val) = self.dispenser() {
            if !_val.validate() {
                return false;
            }
        }
        if let Some(_val) = self.dispenser_instruction() {
            if !_val.into_iter().map(|e| e.validate()).all(|x| x == true) {
                return false;
            }
        }
        if let Some(_val) = self.dose_administration_aid() {
            if !_val.validate() {
                return false;
            }
        }
...
impl MedicationRequest_DispenseRequestBuilder {
    ...
    pub fn dispenser<'a>(
        ...
        self.value["dispenser"] = json!(val.value);
        return self;
    }

    pub fn dispenser_instruction<'a>(
        &'a mut self,
        val: Vec<Annotation>,
    ) -> &'a mut MedicationRequest_DispenseRequestBuilder {
        self.value["dispenserInstruction"] =
            json!(val.into_iter().map(|e| e.value).collect::<Vec<_>>());
        return self;
    }

    pub fn dose_administration_aid<'a>(
        &'a mut self,
        val: CodeableConcept,
    ) -> &'a mut MedicationRequest_DispenseRequestBuilder {
        self.value["doseAdministrationAid"] = json!(val.value);
        return self;
    }
