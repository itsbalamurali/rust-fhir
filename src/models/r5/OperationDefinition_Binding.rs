...
impl OperationDefinition_Binding<'_> {
    ...
    pub fn strength(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("strength") {
            return Some(string);
        }
        return None;
    }
...
impl OperationDefinition_BindingBuilder {
    ...
    pub fn strength<'a>(&'a mut self, val: &str) -> &'a mut OperationDefinition_BindingBuilder {
        self.value["strength"] = json!(val);
        return self;
    }
}
