...
use crate::models::r5::PackagedProductDefinition_LegalStatusOfSupply::PackagedProductDefinition_LegalStatusOfSupply;
use crate::models::r5::PackagedProductDefinition_Package::PackagedProductDefinition_Package;
...
impl PackagedProductDefinition<'_> {
    ...
    /// even when these are different manufactured items. For example a pill pack of 21
    /// tablets plus 7 sugar tablets, can be denoted here as '28 tablets'. This attribute
    /// is repeatable so that the different item types in one pack type can be counted
    ...
    /// is not intended to allow different counts of similar items (e.g. not '2 tubes and
    /// 3 tubes'). Repeats are not to be used to represent different pack sizes (e.g. 20
    /// pack vs. 50 pack) - which would be different instances of this resource.
    ...
    /// Manufacturer of this package type. When there are multiple it means these are all
    ...
    /// A name for this package. Typically what it would be listed as in a drug formulary
    ...
    /// a device or a medication manufactured item).
