...
use crate::models::r5::PlanDefinition_Action::PlanDefinition_Action;
use crate::models::r5::PlanDefinition_Actor::PlanDefinition_Actor;
use crate::models::r5::PlanDefinition_Goal::PlanDefinition_Goal;
...
impl PlanDefinition<'_> {
    ...
    /// clinical care, an action would be to prescribe a particular indicated medication,
    /// or perform a particular test as appropriate. In pharmaceutical quality, an action
    /// would be the test that needs to be performed on a drug product as defined in the
    ...
    pub fn actor(&self) -> Option<Vec<PlanDefinition_Actor>> {
        if let Some(Value::Array(val)) = self.value.get("actor") {
            return Some(
                val.into_iter()
                    .map(|e| PlanDefinition_Actor {
                        value: Cow::Borrowed(e),
                    })
                    .collect::<Vec<_>>(),
            );
    ...
    /// Explanation of why this plan definition is needed and why it has been designed as
    /// it has.
    ...
    /// address at which at which an authoritative instance of this plan definition is (or
...
impl PlanDefinitionBuilder {
    ...
    pub fn _subject_canonical<'a>(&'a mut self, val: Element) -> &'a mut PlanDefinitionBuilder {
    ...
    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut PlanDefinitionBuilder {
        self.value["status"] = json!(val);
        return self;
    }

    pub fn subject_canonical<'a>(&'a mut self, val: &str) -> &'a mut PlanDefinitionBuilder {
