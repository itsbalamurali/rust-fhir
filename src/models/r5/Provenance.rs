...
use crate::models::r5::Provenance_Agent::Provenance_Agent;
use crate::models::r5::Provenance_Entity::Provenance_Entity;
...
impl Provenance<'_> {
    ...
    /// recorded.
    pub fn authorization(&self) -> Option<Vec<CodeableReference>> {
    ...
    /// resource. A provenance can point to more than one target if multiple resources
    /// were created/updated by the same activity.
...
impl ProvenanceBuilder {
    ...
    ) -> &'a mut ProvenanceBuilder {
        self.value["authorization"] = json!(val.into_iter().map(|e| e.value).collect::<Vec<_>>());
        return self;
    }

    pub fn based_on<'a>(&'a mut self, val: Vec<Reference>) -> &'a mut ProvenanceBuilder {
    ...
    pub fn encounter<'a>(&'a mut self, val: Reference) -> &'a mut ProvenanceBuilder {
