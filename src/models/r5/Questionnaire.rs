...
use crate::models::r5::Questionnaire_Item::Questionnaire_Item;
...
impl Questionnaire<'_> {
    ...
    /// which an authoritative instance of this questionnaire is (or will be) published.
    /// This URL can be the target of a canonical reference. It SHALL remain the same when
    ...
    /// value managed by the questionnaire author and is not expected to be globally
...
impl QuestionnaireBuilder {
    ...
    pub fn author<'a>(&'a mut self, val: Vec<ContactDetail>) -> &'a mut QuestionnaireBuilder {
    ...
    pub fn editor<'a>(&'a mut self, val: Vec<ContactDetail>) -> &'a mut QuestionnaireBuilder {
    ...
    pub fn endorser<'a>(&'a mut self, val: Vec<ContactDetail>) -> &'a mut QuestionnaireBuilder {
    ...
    pub fn reviewer<'a>(&'a mut self, val: Vec<ContactDetail>) -> &'a mut QuestionnaireBuilder {
        self.value["reviewer"] = json!(val.into_iter().map(|e| e.value).collect::<Vec<_>>());
        return self;
    }

    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut QuestionnaireBuilder {
    ...
    pub fn topic<'a>(&'a mut self, val: Vec<CodeableConcept>) -> &'a mut QuestionnaireBuilder {
