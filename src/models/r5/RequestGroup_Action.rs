...
use crate::models::r5::RequestGroup_Condition::RequestGroup_Condition;
use crate::models::r5::RequestGroup_Participant::RequestGroup_Participant;
use crate::models::r5::RequestGroup_RelatedAction::RequestGroup_RelatedAction;
...
impl RequestGroup_Action<'_> {
    ...
    pub fn participant(&self) -> Option<Vec<RequestGroup_Participant>> {
        if let Some(Value::Array(val)) = self.value.get("participant") {
            return Some(
                val.into_iter()
                    .map(|e| RequestGroup_Participant {
                        value: Cow::Borrowed(e),
                    })
                    .collect::<Vec<_>>(),
            );
...
impl RequestGroup_ActionBuilder {
    ...
    pub fn _link_id<'a>(&'a mut self, val: Element) -> &'a mut RequestGroup_ActionBuilder {
    ...
    pub fn goal<'a>(&'a mut self, val: Vec<Reference>) -> &'a mut RequestGroup_ActionBuilder {
    ...
    pub fn link_id<'a>(&'a mut self, val: &str) -> &'a mut RequestGroup_ActionBuilder {
    ...
    pub fn participant<'a>(
        &'a mut self,
        val: Vec<RequestGroup_Participant>,
    ) -> &'a mut RequestGroup_ActionBuilder {
