...
use crate::models::r5::ResearchSubject_Progress::ResearchSubject_Progress;
...
impl ResearchSubject<'_> {
    ...
    /// appropriate.
    pub fn progress(&self) -> Option<Vec<ResearchSubject_Progress>> {
        if let Some(Value::Array(val)) = self.value.get("progress") {
            return Some(
                val.into_iter()
                    .map(|e| ResearchSubject_Progress {
                        value: Cow::Borrowed(e),
                    })
                    .collect::<Vec<_>>(),
            );
...
impl ResearchSubjectBuilder {
    ...
    pub fn new(study: Reference, subject: Reference) -> ResearchSubjectBuilder {
    ...
    pub fn progress<'a>(
        &'a mut self,
        val: Vec<ResearchSubject_Progress>,
    ) -> &'a mut ResearchSubjectBuilder {
        self.value["progress"] = json!(val.into_iter().map(|e| e.value).collect::<Vec<_>>());
        return self;
    }

    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut ResearchSubjectBuilder {
