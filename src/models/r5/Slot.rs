...
impl Slot<'_> {
    ...
    /// type).
    pub fn appointment_type(&self) -> Option<Vec<CodeableConcept>> {
        if let Some(Value::Array(val)) = self.value.get("appointmentType") {
            return Some(
                val.into_iter()
...
impl SlotBuilder {
    ...
    pub fn appointment_type<'a>(&'a mut self, val: Vec<CodeableConcept>) -> &'a mut SlotBuilder {
        self.value["appointmentType"] = json!(val.into_iter().map(|e| e.value).collect::<Vec<_>>());
        return self;
    }
    ...
    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut SlotBuilder {
