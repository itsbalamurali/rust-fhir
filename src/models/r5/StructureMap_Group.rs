...
use crate::models::r5::StructureMap_Input::StructureMap_Input;
use crate::models::r5::StructureMap_Rule::StructureMap_Rule;
use serde_json::json;
...
impl StructureMap_Group<'_> {
    ...
    /// of types.
    pub fn type_mode(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("typeMode") {
            return Some(string);
        }
        return None;
    }
...
impl StructureMap_GroupBuilder {
    ...
    pub fn new(input: Vec<StructureMap_Input>) -> StructureMap_GroupBuilder {
    ...
    pub fn rule<'a>(
        &'a mut self,
        val: Vec<StructureMap_Rule>,
    ) -> &'a mut StructureMap_GroupBuilder {
        self.value["rule"] = json!(val.into_iter().map(|e| e.value).collect::<Vec<_>>());
        return self;
    }

    pub fn type_mode<'a>(&'a mut self, val: &str) -> &'a mut StructureMap_GroupBuilder {
        self.value["typeMode"] = json!(val);
        return self;
    }
}
