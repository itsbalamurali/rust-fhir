...
impl Subscription<'_> {
    ...
    /// When multiple filters are applied, evaluates to true if all the conditions are
    ...
    /// an the interval period equal to this elements integer value in seconds.    If
    ...
    /// trigger resources in a single notification).  Note that this is not a strict
    /// limit on the number of entries in a bundle, as dependent resources can be
    /// included.
