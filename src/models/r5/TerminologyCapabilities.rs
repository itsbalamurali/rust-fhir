...
use crate::models::r5::TerminologyCapabilities_Closure::TerminologyCapabilities_Closure;
use crate::models::r5::TerminologyCapabilities_CodeSystem::TerminologyCapabilities_CodeSystem;
use crate::models::r5::TerminologyCapabilities_Expansion::TerminologyCapabilities_Expansion;
use crate::models::r5::TerminologyCapabilities_Implementation::TerminologyCapabilities_Implementation;
use crate::models::r5::TerminologyCapabilities_Software::TerminologyCapabilities_Software;
use crate::models::r5::TerminologyCapabilities_Translation::TerminologyCapabilities_Translation;
use crate::models::r5::TerminologyCapabilities_ValidateCode::TerminologyCapabilities_ValidateCode;
...
impl TerminologyCapabilities<'_> {
    ...
    /// The degree to which the server supports the code search parameter on ValueSet, if
    /// it is supported.
    pub fn code_search(&self) -> Option<&str> {
        if let Some(Value::String(string)) = self.value.get("codeSearch") {
            return Some(string);
        }
        return None;
    }
    ...
    /// A formal identifier that is used to identify this terminology capabilities when it
    ...
    /// A legal or geographic region in which the terminology capabilities is intended to
    /// be used.
    ...
    /// capabilities is (or will be) published. This URL can be the target of a canonical
    /// reference. It SHALL remain the same when the terminology capabilities is stored on
    /// different servers.
    ...
    /// references to specific programs (insurance plans, studies, ...) and may be used
    /// to assist with indexing and searching for appropriate terminology capabilities
    /// instances.
    ...
    /// capabilities when it is referenced in a specification, model, design or instance.
    /// This is an arbitrary value managed by the terminology capabilities author and is
    /// not expected to be globally unique. For example, it might be a timestamp (e.g.
    /// yyyymmdd) if a managed version is not available. There is also no expectation that
...
impl TerminologyCapabilitiesBuilder {
    ...
    pub fn code_search<'a>(&'a mut self, val: &str) -> &'a mut TerminologyCapabilitiesBuilder {
        self.value["codeSearch"] = json!(val);
        return self;
    }
    ...
    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut TerminologyCapabilitiesBuilder {
