...
use crate::models::r5::ValueSet_Compose::ValueSet_Compose;
use crate::models::r5::ValueSet_Expansion::ValueSet_Expansion;
use crate::models::r5::ValueSet_Scope::ValueSet_Scope;
use serde_json::json;
...
impl ValueSet<'_> {
    ...
    pub fn scope(&self) -> Option<ValueSet_Scope> {
        if let Some(val) = self.value.get("scope") {
            return Some(ValueSet_Scope {
                value: Cow::Borrowed(val),
            });
        }
        return None;
...
impl ValueSetBuilder {
    ...
    pub fn scope<'a>(&'a mut self, val: ValueSet_Scope) -> &'a mut ValueSetBuilder {
        self.value["scope"] = json!(val.value);
        return self;
    }

    pub fn status<'a>(&'a mut self, val: &str) -> &'a mut ValueSetBuilder {
