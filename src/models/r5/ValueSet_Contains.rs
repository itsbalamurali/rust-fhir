...
use crate::models::r5::ValueSet_Designation::ValueSet_Designation;
use crate::models::r5::ValueSet_Property1::ValueSet_Property1;
use serde_json::json;
...
impl ValueSet_Contains<'_> {
    ...
    pub fn property(&self) -> Option<Vec<ValueSet_Property1>> {
        if let Some(Value::Array(val)) = self.value.get("property") {
            return Some(
                val.into_iter()
                    .map(|e| ValueSet_Property1 {
                        value: Cow::Borrowed(e),
                    })
                    .collect::<Vec<_>>(),
            );
