...
use crate::models::r5::ValueSet_Contains::ValueSet_Contains;
use crate::models::r5::ValueSet_Parameter::ValueSet_Parameter;
use crate::models::r5::ValueSet_Property::ValueSet_Property;
use serde_json::json;
...
impl ValueSet_Expansion<'_> {
    ...
    pub fn property(&self) -> Option<Vec<ValueSet_Property>> {
        if let Some(Value::Array(val)) = self.value.get("property") {
            return Some(
                val.into_iter()
                    .map(|e| ValueSet_Property {
                        value: Cow::Borrowed(e),
                    })
                    .collect::<Vec<_>>(),
            );
